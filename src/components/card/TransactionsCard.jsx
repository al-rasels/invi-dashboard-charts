...
export default function TransactionsCard({ transactions }) {
  ...
  return (
    <div className="glass-panel !bg-white/95 !rounded-3xl !shadow-sm hover:!shadow-xl !transition-all !duration-500 !p-6 md:!p-8 h-full border border-slate-200/70 flex flex-col">
      <div className="flex justify-between items-start mb-8">
        <div>
          <h2 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">Recent <span className="text-indigo-600">Transactions</span></h2>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Live Sales Feed</p>
        </div>
        <button className="!p-2.5 bg-slate-100/90 !rounded-xl text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all">
      ...
      <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar  max-h-[500px]">
        {displayTransactions.map((item, i) => (
          <div key={i} className={`flex justify-between items-center !p-4 !rounded-2xl border border-slate-200/70 bg-slate-50/80 transition-all hover:shadow-md hover:border-indigo-100 group cursor-pointer`}>
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <p className="font-black text-slate-900 text-sm md:text-base truncate max-w-[150px]">{item.title}</p>
                <div className="flex items-center gap-2">
                  <p className="text-slate-500 text-[10px] md:text-xs font-bold uppercase tracking-widest">{item.subtitle}</p>
